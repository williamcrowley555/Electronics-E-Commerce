...
public class InvoiceDAL extends AbstractDAL<InvoiceDTO> implements IInvoiceDAL {
    ...
    public void update(InvoiceDTO invoice) {
        String sql = "UPDATE invoice SET address = ? cancelling_date = ? confirmation_date = ? order_date = ? payment_date = ? phone = ? recipient_first_name = ?"
                + "recipient_last_name = ? ship_date = ? status = ? status = ? total = ? user_id = ? "
                + "WHERE id = ?";
        update(sql, invoice.getAddress(), invoice.getCancellingDate(), invoice.getConfirmationDate(), invoice.getOrderDate(), invoice.getPaymentDate(),
                     invoice.getPhone(), invoice.getRecipientFirstName(), invoice.getRecipientLastName(), invoice.getShipDate(), invoice.getStatus(),
